  ...
  try {
    ...
    Object.keys(installedList).map(async component => {
      ...
      var data = JSON.stringify({
        ...
        variables: {
          id: packageToComponent(installedList[component]),
        },
      ...
      await axios(config)
        .then(function(response) {
          parameters =
            response.data['data']['getHost']['getDocs']['properties'];
        })
        .catch(function(error) {
          console.log(error);
        });
      ...
      const paramList = JSON.parse(paramString);
      const panelIcCode = await generateICPanel(component, paramList);

      fs.writeFileSync(
        `src/installed-components/${component}Panel.ic.tsx`,
        panelIcCode,
      );
    });
